...
import { CAR_LENGTH, CAR_WIDTH, type CarStats } from "../core/tuning.ts";
...
/**
 * Build the bodies for a track. The car is a real Matter.js body (rectangle
 * with Matter geometry/bounds); the track is a ring of static wall rectangles.
 *
...
export function createCarWorld(track: BuiltTrack): CarWorld {
  const engine = M.Engine.create(); // kept for the IRenderer/Matter seam
  engine.gravity.x = 0;
  engine.gravity.y = 0;
